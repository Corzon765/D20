...
const SHEET_URL = "https://script.google.com/macros/s/AKfycbxJJ8BmQMutS8hZQy3Itbhc5Mca1e5ZfeGBktc84OFfSg5eoYv9qvvZSey7Vz4-ntKq/exec";
...
async function loadData() {
  try {
    ...
    if (!contentTypeMain || !contentTypeMain.includes("application/json")) {
      ...
      console.error("Error: El servidor devolvió HTML en lugar de JSON.", text.substring(0, 100));
      throw new Error("El servidor devolvió HTML. Verifica la implementación de Google Apps Script.");
    }
    if (!contentTypeItems || !contentTypeItems.includes("application/json")) {
      const text = await resItems.text();
      console.error("Error: El servidor de Items devolvió HTML.", text.substring(0, 100));
      throw new Error("El servidor de Items devolvió HTML.");
    }

    // 4. Convertimos la respuesta a JSON (¡Aquí estaba el error!)
    // La variable se llama 'raw', NO 'jsonMain'
    let raw = await resMain.json(); 
    
    // 5. Procesar Datos Principales (PC/Mob)
    // Si la respuesta viene envuelta en { data: [...] }, extraemos el array
    if (raw && Array.isArray(raw.data)) {
      raw = raw.data;
    } else if (!Array.isArray(raw)) {
      // Si no es array, intentamos convertir a array de valores
